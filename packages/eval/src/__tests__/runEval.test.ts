...
import { CHUNKS, DATASET, fixtureResult, mockEmbeddingClient } from "./fixtures.js";
import type { RagAnswer } from "@codeflow/analyzers";
import type { AnswerRunner } from "../runEval.js";
...
function answerRunner(overrides: Partial<RagAnswer> = {}): AnswerRunner {
  return async (question: string) => {
    ...
    const chunk = CHUNKS.find((entry) => entry.fileId === target);
...
describe("runEval — answer-path scoring", () => {
  ...
  it("scores citation validity, relevance and refusals when a runner IS supplied", async () => {
    const report = await runEval(DATASET, fixtureResult(), mockEmbeddingClient(), { answer: answerRunner() });
  ...
  it("fails the citation-validity gate when the answer cites code it never retrieved", async () => {
    ...
    const report = await runEval(DATASET, fixtureResult(), mockEmbeddingClient(), { answer: fabricating });
  ...
  it("stays deterministic with the answer path enabled", async () => {
    const first = await runEval(DATASET, fixtureResult(), mockEmbeddingClient(), { answer: answerRunner() });
    const second = await runEval(DATASET, fixtureResult(), mockEmbeddingClient(), { answer: answerRunner() });
