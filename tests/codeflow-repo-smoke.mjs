...
const htmlSource = await readFile(join(repoRoot, 'legacy', 'index.html'), 'utf8');
...
if (!paths.length) {
  // This is a CLI utility (analyze a local repo dir), NOT a unit test. When it gets swept
  // up by `node --test tests/*.mjs` it runs with no args — skip-with-reason and exit 0 so
  // the default suite is honestly green, instead of failing with a usage error. Pass one or
  // more <repo-dir> args to actually run it. No live services / clone are involved.
  console.error('SKIP codeflow-repo-smoke: CLI utility — pass <repo-dir>... to run (analyzes a local clone you already have).');
  process.exit(0);
}
