...
function locateIndexHtml(actionDir, repoRoot) {
  // The legacy single-file app was quarantined to legacy/index.html during the MERN
  // cleanup, but older checkouts may still have it at the repo root. Try both, in both
  // the Action-adjacent and repo-root locations.
  const candidates = [
    path.join(actionDir, '..', 'legacy', 'index.html'),
    path.join(actionDir, '..', 'index.html'),
    path.join(repoRoot, 'legacy', 'index.html'),
