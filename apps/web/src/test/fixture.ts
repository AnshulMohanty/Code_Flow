...
/**
 * A TEST-ONLY fixture: a faithful `AnalysisResult` + `ProgressEvent` payload for exercising the pure
 * read-models (`graphModel`, `siteModel`, `architecture`) and the two shipped surfaces against a
 * realistic shape -- including a PARTIAL run whose RAG stage was skipped by the daily-budget guard,
 * which is what makes it useful for the honest-degradation assertions.
 *
 * IT LIVES UNDER src/test/ ON PURPOSE (moved here in V3-FINAL). It used to sit in `src/lib/` beside
 * the read-models, and a SHIPPED code path imported it: the old shell had a "Use Mock Data Instead"
 * button that loaded these fabricated module names and metrics into every dashboard view. That is a
 * direct violation of the invariant the whole product rests on -- a user could not tell this from a
 * real analysis -- and the mock path is now gone.
 *
 * The fixture itself was never the problem; the shipped consumer was. Keeping it under `src/test/`
 * makes the boundary structural rather than a matter of discipline: nothing in a production view has
 * a reason to import from a test directory, and a reviewer can see the violation in the import path.
 */
